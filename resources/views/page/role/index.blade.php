...
@section ('contents')
	<div class="panel panel-headline">
		<div class="panel-heading">
			<h3 class="panel-title"><b>Roles</b></h3>
				...
				<div class="panel-body">
					<div class="btn-group pull-right">
						...
						<ul class="dropdown-menu action-menu">
							<li><a href="{{ route('role.create') }}" class="action-list"><span class="fa fa-check"></span>
							 Create</a></li>
								...
								<tbody>
									@if ( count($roles) )
										@foreach ($roles as $role)
											<tr class="row-data" data-id="{{$role->id}}" data-name="{{$role->name}}">
												<td><span class="column-name">{{ $role->name }}</span> &nbsp;&nbsp;&nbsp;&nbsp;
													<span class="hidden-menu hide">
														<a href="{{ route('role.edit', $role->id) }}">
															<i class="fa fa-edit"></i> Edit
														</a>
														&nbsp;
														<a href="{{ route('role.destroy', $role->id) }}" class="swal-confirm trashed-link" data-title="Do you really want to delete this item?" data-message="{{$role->name}}" data-type="error">
